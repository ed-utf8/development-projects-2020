...
public class AdminChatCommand implements CommandExecutor {
    ...
    public AdminChatCommand(CorePlugin corePlugin, UserManager userManager, RedisManager redisManager) {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String label, String[] arguments) {
        ...
        if (arguments.length < 1) {
            commandSender.sendMessage(corePlugin.getPrefix() + "Usage: /adminchat <message>");
            return true;
        }
        ...
        redisManager.sendAdminChat(coreUser.getUserName(), Joiner.on(" ").skipNulls().join(arguments));
